	...
	private void setMap() {
		...
		while(cnt>0) {
			...
			if(this.map[rX][rY].getItem() == 0) {
				int check = -1;
				for(int i=0; i<this.snake.length; i++) {
					if(this.snake[i].getX()==this.map[rX][rY].getX() && this.snake[i].getY()==this.map[rX][rY].getY()) {
						check = 1;
						break;
					}
				}
	...
	public void keyPressed(KeyEvent e) {
		if(e.getKeyCode() == e.VK_LEFT) { // x-, 좌측에 몸이 존재할때
			if(!dead && this.PX>0 && this.snake[0].getX()-50 != this.snake[1].getX()) {
				...
				for(int i=0; i<SIZE; i++) {
					for(int j=0; j<SIZE; j++) {
						if(this.map[i][j].getItem()==1 && this.snake[0].getX()-50 == this.map[i][j].getX() && this.snake[0].getY() == this.map[i][j].getY()) {
							checkX = i;
							checkY = j;
							break;
						}
				...
				if(checkX==-1 && checkY==-1) { // item X
					...
					for(int i=0; i<this.snake.length; i++) {
						if(i==0) {
							this.snake[i] = new SkRect(this.map[checkX][checkY].getX(), this.map[checkX][checkY].getY());
						}
						else {
							this.snake[i] = new SkRect(temp[i-1].getX(), temp[i-1].getY());
						}
			...
			if(!dead && this.PY < SIZE-1 && this.snake[0].getY()+50 != this.snake[1].getY()) {
				...
				for(int i=0; i<SIZE; i++) {
					for(int j=0; j<SIZE; j++) {
						if(this.map[i][j].getItem()==1 && this.snake[0].getX() == this.map[i][j].getX() && this.snake[0].getY()+50 == this.map[i][j].getY()) {
							checkX = i;
							checkY = j;
							break;
						}
		...
		for(int i=0; i<SIZE; i++) {
			for(int j=0; j<SIZE; j++) {
				...
				if(temp.getItem()==1) {
					...
					g.fillRoundRect(temp.getX()+8, temp.getY()+8, temp.getW()-15, temp.getH()-15, temp.getW()-15, temp.getH()-15);
				}
			}
		}
