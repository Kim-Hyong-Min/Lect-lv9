...
class Omok2Panel extends JPanel implements MouseListener{
		...
		for(int i=0; i<SIZE; i++) {
			for(int j=0; j<SIZE; j++) {
				...
				if(r.getOwner() != 0) {
					...
					g.drawRoundRect(r.getX()+5, r.getY()+5, r.getW()-10, r.getH()-10, r.getW(), r.getH()); // 태두리
					
					g.setColor(r.getC());
					g.fillRoundRect(r.getX()+5, r.getY()+5, r.getW()-10, r.getH()-10, r.getW(), r.getH()); // 안쪽
				}
			}
		}
			...
			for(int j=0; j<this.map.length; j++) {
				...
				if(x >= r.getX() && x < r.getX() + r.getW() && y >= r.getY() && y < r.getY() + r.getH()) {
					if(r.getOwner() == 0) {
						...
						r.setC(this.turn == 1 ? this.p1 : this.p2); // 삼항연산자
						
						checkWin();
						
						this.turn = this.turn == 1 ? 2 : 1;// 삼항연산자
					}
				}
			}
		}
	...
	private void checkWin() {
		...
		if(this.win != 0) {
			System.out.printf("p%d win/n",this.win);
			new OmokResult(String.format("p%d win/n",this.win));
		}
	}
...
public class 문제04풀이 extends JFrame{
